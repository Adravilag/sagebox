 ...
 *
 * Generated by SagedUI Vite Plugin
...
/**
 * SagedUI Icons Vite Plugin
 */
function sagedUIIcons(userOptions = {}) {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.watcher.on('change', (changedPath) => {
        if (changedPath.endsWith('.svg') || changedPath.endsWith('icons.json')) {
          console.log(`\n🎨 [saged-ui] Icon changed: ${path.basename(changedPath)}`);
          generateIconsFile(iconsDir, jsonPath, outputPath, options);
      ...
      server.watcher.on('add', (addedPath) => {
        if (addedPath.endsWith('.svg')) {
          console.log(`\n🎨 [saged-ui] Icon added: ${path.basename(addedPath)}`);
          generateIconsFile(iconsDir, jsonPath, outputPath, options);
        }
      });
      ...
      server.watcher.on('unlink', (removedPath) => {
        if (removedPath.endsWith('.svg')) {
          console.log(`\n🎨 [saged-ui] Icon removed: ${path.basename(removedPath)}`);
          generateIconsFile(iconsDir, jsonPath, outputPath, options);
        }
      });
    },
    ...
    resolveId(id) {
      if (id === 'virtual:saged-ui-icons') {
        return '\0virtual:saged-ui-icons';
      }
    },

    load(id) {
      if (id === '\0virtual:saged-ui-icons') {
        const iconsDir = path.resolve(root, options.iconsDir);
        const jsonPath = path.join(iconsDir, options.jsonFile);
...
function generateIconsFile(iconsDir, jsonPath, outputPath, options) {
  try {
    ...
    console.log(`🎨 [saged-ui] Generated ${Object.keys(icons).length} icons → ${path.relative(process.cwd(), outputPath)}`);
  } catch (e) {
    console.error(`🎨 [saged-ui] Error: ${e.message}`);
  }
}

module.exports = { sagedUIIcons };
module.exports.default = sagedUIIcons;
